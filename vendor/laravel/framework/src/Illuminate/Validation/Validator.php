...
class Validator implements MessageProviderInterface {
     ...
     * @return bool
     */
    protected function validateAccount($attribute, $value)
    {
    ...
    /**
     * Validate that an attribute contains only alpha-numeric characters, dashes, utf8 and underscores.
     *
     * @param  string  $attribute
     * @param  mixed   $value
     * @return bool
     */
    protected function validateAlphaExtra($attribute, $value)
    {
        $str = utf8_encode($value);
        return ( ! preg_match("/^[[:alpha:]- 1234567890\/,_-ÀÁẢÃẠÂẦẤẨẪẬĂẰẮẲẴẶÄÅĀĄĂÆÇĆČĈĊĎĐÈÉẺẼẸÊỀẾỂỄỆËĒĘĚĔĖĜĞĠĢĤĦÌÍỈĨỊÎÏĪĨĬĮİĲĴĶŁĽĹĻĿÑŃŇŅŊÒÓỎÕỌÔỒỐỔỖỖƠỜỚỞỠỢÕÖØŌŐŎŒŔŘŖŚŠŞŜȘŤŢŦȚÙÚŨỤƯỪỨỬỮỰÛÜŪŮŰŬŨŲŴÝỲỶỸỴŶŸŹŽŻàáảãạâầấẩẫậăằắẳẵặãäåæçèéẻẽẹêềếểễệëìíỉĩịîïñòóỏõọôồốổỗộơờớởỡợõöøùúủũũưừứửữựûüýỳỷỹỵÿœšß_.]+$/", $str)) ? FALSE : TRUE;
    }
